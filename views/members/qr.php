...
(() => {
  ...
  const renderQr = (content) => {
    ...
    // Admin canvas (large, dark-on-white, high error correction for reliability)
    clearEl(canvasWrap);
    try {
      const qrSize = getQrSize();
      // Use correctLevel.H (30% error correction) for better resilience to damage/compression
    ...
    // Card QR (135px for business card — optimized for reliable scanning, white-on-dark)
    // Uses HIGH error correction (30%) to survive print degradation and screenshot compression
    if (cardQrWrap) {
      clearEl(cardQrWrap);
      try {
        new window.QRCode(cardQrWrap, { text, width: 135, height: 135,
...
.bcard-brand-name {
  font-size: 6.5pt;         /* Compact: 6.5pt for small header */
  font-weight: 700;
...
.bcard-brand-sub {
  font-size: 4pt;           /* Compact: 4pt for minimal header */
  color: #666666;
  margin: 0;
...
.bcard-qr canvas {
  ...
  width:   135px   !important;   /* Larger QR: excellent for scanning, fills more of card */
